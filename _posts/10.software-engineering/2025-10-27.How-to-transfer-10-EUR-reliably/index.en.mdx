...
tags: ['software', 'web', 'architecture']
---
...
<TLDR title="With idempotency it's trivial, without — impossible" />

- In general case, this seemingly routine task is a distributed consensus problem
- Why I consider saga an anti-pattern
- Transaction Log could be used for recovery but most likely the solution cannot be fully automated
...
If you think that what I'm saying is overcomplicated, it's because it is.
When I think about practical applications of consistency and reliability, I like to recall the issue with PostgreSQL using Linux's `fsync` syscall incorrectly for over a decade.
...
So, "It's in an ACID database" comes with a few asterisks.
