...
sequenceDiagram
    ...
    User->>OurServer: withdraw (amount: 10)
    activate OurServer

    OurServer->>Network: send request
    activate Network

    Network->>RemoteServer: withdraw (amount: 10)
    activate RemoteServer

    RemoteServer->>RemoteServer: process withdrawal

    RemoteServer-->>Network: respond (success/failure)
    deactivate RemoteServer

    Network-->>OurServer: respond (success/failure)
    deactivate Network

    OurServer->>User: store the response
    deactivate OurServer
```
