...
- **`next build` crashed twice with a bare `uncaughtException [TypeError: Cannot read properties of undefined (reading 'length')]`** (2026-08-26, Next 15.5.12) and then passed repeatedly with identical inputs, including from a cold `.next`. No stack trace is printed because the exception is swallowed by the build worker; if it reappears, rerun with `NODE_OPTIONS=--trace-uncaught` to capture the origin.
