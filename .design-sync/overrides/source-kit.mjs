...
export async function resolvePackage(ctx) {
  ...
  const fuzzyFind = (name) => {
    ...
    const exportRx = new RegExp(`export\\s+(?:default\\s+)?(?:const|let|var|function|class)\\s+${name}\\b`);
  ...
  if (srcRoot) {
    for (const c of components) {
      const hit = typeof srcMap[c.name] === 'string' ? slash(resolve(PKG_DIR, srcMap[c.name])) : fuzzyFind(c.name);
  ...
  if (synthEntry) {
    const matchedPaths = new Set(components.filter((c) => c.srcPath).map((c) => c.srcPath));
    ...
    for (const c of components) {
      ...
      lines.push(`export const ${c.name} = ${alias}.default ?? ${alias}[${JSON.stringify(c.name)}];`);
    }
