// forked from design-sync lib/dts.mjs — two independent fixes:
// 1. allow the all-caps component TLDR (explicit componentSrcMap pin) past the enum-name heuristic.
// 2. THE REAL FIX: this repo has no dist/.d.ts at all (synth-entry mode, cfg.entry
//    is a decoy — see NOTES.md "Source shape"). Upstream loadDts()/propsBodyFor()
//    only ever look for `<typesRoot>/**/*.d.ts`; with zero .d.ts files anywhere in
//    the repo, `projectFor()`'s ts-morph Project loads ZERO source files, every
//    `<Name>Props` lookup finds nothing, and every component silently falls back to
//    the `{ [key: string]: unknown }` stub in emit.mjs. Fixed by: (a) projectFor()
//    additionally loading the *.tsx/*.ts sources straight from cfg.srcDir (read from
//    .design-sync/config.json) through cfg.tsconfig (so the `@/*` path aliases and
//    Next.js/node-builtin shims resolve) whenever the .d.ts glob comes up empty; (b)
//    propsBodyFor()'s existing entry-based call-signature fallback extended to scan
//    every in-package source file (not just a nonexistent dist entry) for a function/
//    variable/forwardRef declaration named `<Name>` — covers components whose props
//    are an inline object-literal annotation with no named `<Name>Props` type (e.g.
//    TableOfContents); (c) typeText() depth-capped to expand exactly one level of any
//    non-global named object/array type (needed because emitted .d.ts files import
//    only `React` — see emit.mjs — so a bare reference to a local type like `Post`/
//    `LoadFailure`/`IconType` would be an unresolved identifier, not valid standalone
//    TS) and a generic unresolved-identifier safety net that collapses anything that
//    still looks like a bare non-global reference to `unknown` rather than ever
//    emitting invalid TS.
...
// Read once: cfg.srcDir/cfg.tsconfig for the synth-entry source fallback below.
// Resolved relative to pkgDir (the repo root — see loadDts's package.json walk-up),
...
function projectFor(pkgDir, typesRoot) {
  ...
  // they do for the bundle build. Falls back to manual compilerOptions (upstream
  // behavior) when no cfg.tsconfig is on disk.
  ...
  // THE FIX (part 0, the actual smoking gun): findTypesRoot() has no dedicated
  // dist dir to point at in synth-entry repos (no build/ts, dist/types, types,
  // lib, or dist/ exists) and falls back to pkgDir itself — the REPO ROOT. That
  // makes this glob recurse into the design-sync OUTPUT dir (`--out ./ds-bundle`,
  // gitignored but very much present on disk after the first build), which
  // already contains one `<Name>.d.ts` stub per component from a prior run.
  // Each stub declares `export interface <Name>Props { [key: string]: unknown; }`
  // — a real, syntactically valid `<Name>Props` interface that satisfies the
  // `sf.getInterface(...)` lookup below and gets found FIRST, before this
  ...
  if (project.getSourceFiles().length === 0) {
    ...
    // to its top-level segment ('src') so sibling modules the props reference
    // (@/lib/blog/types, @/i18n, …) are in the same ts-morph project too.
    const srcTop = (cfg?.srcDir ?? 'src').split('/')[0] || 'src';
    const srcRoot = join(pkgDir, srcTop);
    project.addSourceFilesAtPaths([
      `${srcRoot}/**/*.tsx`,
      `${srcRoot}/**/*.ts`,
      `!${srcRoot}/**/*.d.ts`,
...
// external one like react-icons' `IconType`) is an unresolved identifier and
// fails validate's [DTS_PARSE] check. There is nowhere to add an import for
// them (emit.mjs's `prelude` slot exists for this, but keeping every referenced
// type's dependency graph in sync there is exactly the kind of open-ended
// recursion the depth cap below avoids). So: expand exactly one level of any
// non-global named object/array/function type into a structural literal
// instead of printing its name; anything nested inside THAT one level collapses
// to `unknown` rather than resolving further (also breaks cycles like
...
// reference to a non-global name. Scan the final text and collapse to
...
export function propsBodyFor(name, ctx) {
  ...
  // Fallback A: derive from the component symbol's first call signature, via
  ...
  // Fallback B (THE FIX, synth-entry mode — no dist entry exists at all): scan
  // every in-package source file directly for a declaration named `<Name>`
  // (named export, or a default export whose file is literally `<Name>.tsx` —
  // this repo's convention, see NOTES.md) and derive props from its first
  // parameter, same as Fallback A but without needing a dist entry module.
  // Needed for components whose prop shape is an inline object-literal
  // annotation with no named `<Name>Props` type at all (e.g. TableOfContents).
  if (!decl) {
    for (const sf of project.getSourceFiles()) {
      const fp = sf.getFilePath();
      if (!fp.startsWith(pkgDir)) continue;
...
// Shared by both call-signature fallbacks above: given a value declaration
// (function/variable/class), find its first parameter's type and flatten it.
// Handles the three shapes this repo's (and most React code's) components
// come in: plain function, `const X = (props) => …` arrow, and
// `const X = React.forwardRef((props, ref) => …)` — forwardRef's OWN type is
// a non-callable exotic-component object (no call signatures), so its render
// callback's first parameter has to be read directly off the call expression.
function callSigPropsBody(exp, pkgDir) {
  let target = exp;
  ...
  const sig = target.getType().getCallSignatures()[0];
  const p0 = sig?.getParameters()[0];
  return p0 ? emitBody(p0.getTypeAtLocation(target), target, '', pkgDir) : null;
}
